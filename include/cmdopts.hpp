...
cxxopts::ParseResult parse(int argc, char* argv[]) {
  ...
  try {
    ...
    options.add_options()
        ...
        ("c,client", "run in client mode, connecting to", cxxopts::value<string>()->default_value("127.0.0.1"), "<host>")
        ("p,port", "server port to listen on/connect to",cxxopts::value<int>()->default_value("40440"),"<port>")
